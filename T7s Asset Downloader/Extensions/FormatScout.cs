    ...
    public class FormatScout
    {
        ...
        public async void Format()
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    var fileNameVer2 = Crypt.ConvertFileName(filePath, EncVersion.Ver2, EncVersion.Ver1);
                    ...
                    foreach (var dialog in dialogList.Where( d =>((JObject)d).ContainsKey("TextArea")))
                    {
                        ...
                        await exportText.WriteLineAsync($"◇ {NowIndex:D4} ◇ {name} :");
                        await exportText.WriteLineAsync(Regex.Matches(dialogueWithoutSymbols, @"\p{P}").Count == dialogueWithoutSymbols.Length
